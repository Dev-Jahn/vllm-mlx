...
class MLXMultimodalLM:
    ...
    def _prepare_native_video_inputs(
        ...
        # Try the processor first, then fall back to its tokenizer.
        # Some processors (e.g. Qwen3-VL) expose apply_chat_template but raise
        # ValueError because they lack an internal chat_template attribute
        ...
        try:
            ...
            if tokenizer is None or not hasattr(tokenizer, "apply_chat_template"):
                raise
            logger.warning(
                f"Processor chat template failed ({e}), retrying on tokenizer"
            )
